...
def draw():
    ...
    Matrix = [[noise(1*cos(TWO_PI*a+1.0*i/Y),1*sin(TWO_PI*a+1.0*j/X)) for j in range(X)] for i in range(Y)]
    #Matrix = [[noise(scl*i+a,scl*j+a)
    ...
    if(horizontal):
        ...
        spaceY = normalizeList(spaceY,height+map(sin(TWO_PI*a),-1,1,0,200))
        param = [0,0]
        for j,row in enumerate(Matrix):
            ...
            for i in row:
                ...
                with pushMatrix():
                    ...
                    #fill([color(255,0,0),color(0,0,0),color(0,0,255),color(0,255,0)][param[1]%4])
                    fill(0)
                    #noFill()
                    strokeWeight(0.5)
                    stroke(0,150)
