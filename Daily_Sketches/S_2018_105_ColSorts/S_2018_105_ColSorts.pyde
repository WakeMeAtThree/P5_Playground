...
#Possible color sort ideas:
# - sorting HSBs
# - sorting RGBS within specified ranges
# - sorting lerps of random amounts of randomly generator colors
# etc.
def setup():
    size(400,400)
    ...
    for j in range(Y):
        ...
        for i in range(X):
            xColors.append(
            [
            random.uniform(0.15,0.12),
            0.8,
            1.0
            ]
            )
        
        yColors.append(xColors)
    yColors = [[colorsys.hsv_to_rgb(*j) for j in i] for i in yColors]
    for i in yColors:
        i.sort(key=lambda (r,g,b):Hilbert_to_int([int(r*255),int(g*255),int(b*255)])  )
    print(yColors)
    noStroke()

    spaceX = width/X
    spaceY = height/Y
    colors = ['#72ffff','#3f79d4']
    
    #Hilbert sorting
    colours.sort(key=lambda (r,g,b):Hilbert_to_int([int(r*255),int(g*255),int(b*255)])  )
    for j in range(Y):
        for i in range(X):
            #param = 1.0*(i+j)/(X+Y-1-1)
            param = 1.0*i/(X-1)
            fill(*yColors[j][i])
            rect(i*spaceX,j*spaceY,spaceX,spaceY) 
def draw():
    pass
"""Color sorting learnt from this wonderful article:
