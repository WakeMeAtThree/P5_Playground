def setup():
    ...
    smooth(8)
    noStroke()
    # colorOptions = [color(lerpColor("#FFFFFF","#0EC0E1",0.35)),
    #                 lerpColor("#000000","#0EC0E1",1.0),
    #                 lerpColor(color(0,200),"#0EC0E1",0.0)]
    colorOptions = ["#00b3eb",
                    "#2b00c7",
                    "#9c00b0",
                    "#d10034",
...
def draw():
    ...
    Mpos = getPositions(M)
    translate(197-0, 124+42-39)
    if(mousePressed): print(mouseX)
    scale(0.5)
    rotateX(PI/6)
    rotateZ(PI/4)
    for index,i in enumerate(zip(Y,Ypos)):
        ...
        for index2,j in  enumerate(zip(M[index],Mpos[index])):
            ...
            shift = 1.0*(index2+index)/(len(Y)+len(M[index]))
            q = [i for i in Quadrant(XPosition,YPosition,
                                     Width,Height,Width,chance,shift).subdivide()]
...
class Quadrant(object):
    def __init__(self,x,y,w,h,T, CHANCE, SHIFT):
        global globalCount 
        self.x = x
        self.y = y
        self.w = w
    ...
    def display(self):
        squishedbox(self.x,self.y,
             self.w,self.h,max([0,self.T*5.40]),self.CHANCE,self.SHIFT)
...
def squishedbox(X,Y,L,W,H,hole,shift,val=0.7):
    circleres = 130
    val = map(sin(TWO_PI*a+shift),-1,1,0,0.8)
    ...
    with pushMatrix():
        ...
        with pushStyle():
            ...
            #dashedLine(center,lerpPVector(center.x,center.y,0))
            for i in points:
                pass
                #dashedLine(i,center)
                #line(i.x,i.y,i.z,
