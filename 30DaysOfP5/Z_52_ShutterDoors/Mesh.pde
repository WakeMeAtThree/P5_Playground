...
class Mesh {
  ...
  void display(float a,float alpha) {
    pushStyle();
    noFill();
    //stroke(0,20);
    //strokeWeight(0.15);
    ...
    for (int i = 0; i < faces.size(); i++) {
      ...
      for (int j = 0; j < faces.get(i).length; j++) {
        ...
        //stroke(lerpColor(#FFFFFF, #000000, map(1-cs(a+param1+param2+alpha), -1, 1, 0, 1)),190);
        fill(lerpColor(#FFFFFF, #000000, map(cs(a+param1+param2+alpha), -1, 1, 0, 1)),190);
