class Morph {
  ...
  void display(float a) {
    ...
    if (option) {
      for (Curve crv : lerpCurveStates(curveStates, a)) {
        crv.display();
      }
    } else {
  ...
  Curve vertsLerp(Curve crv1, Curve crv2, float amt) {
    ...
    for (int i = 0; i < crv1.vertices.size(); i++) {
      float param = 9.0*i/crv1.vertices.size();
      blendVerts.add(PVector.lerp(crv1.vertices.get(i), crv2.vertices.get(i), map(sin(amt+param),-2,2,0,1)));
    }
